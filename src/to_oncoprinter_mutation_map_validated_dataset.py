...
This script extracts mutation data for a specific gene from the short_variants.csv file
and converts it to the OncoprinterValidated format. The output is saved as a tab-delimited
text file in the data/oncoprinter directory with the gene name as the filename.
...
def main():
    parser = argparse.ArgumentParser(description='Convert genetic data for a specific gene to OncoprinterValidated format')
    parser.add_argument('--gene', type=str, required=True,
                        help='Gene name to extract data for')
    ...
    output_file = os.path.join(output_dir, f"{args.gene}.txt")
    
    # Convert and write data
    convert_to_oncoprinter_format(variants, args.gene, output_file)
