#!/usr/bin/env python3
"""
to_oncoprinter_validated_dataset.py

This script converts genetic data from CSV files in the data directory to the OncoprinterValidated format.
It follows the format rules for genomic data as specified in the OncoprinterValidated documentation.
...
def convert_to_oncoprinter_format(patients, variants, cnas, rearrangements, output_file):
    ...
    with open(output_file, 'w', newline='') as f:
        ...
        # Write unaltered patients (Sample only format)
        # NOTE: We're commenting this out as it might be causing issues with the Oncoprinter tool
        # for patient_id in all_patient_ids:
        #     writer.writerow([patient_id])

def main():
    parser = argparse.ArgumentParser(description='Convert genetic data to OncoprinterValidated format')
    ...
    parser.add_argument('--output', type=str, default=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'oncoprinter', 'all.txt'),
                        help='Output file name for the OncoprinterValidated format data')
    ...
    convert_to_oncoprinter_format(patients, variants, cnas, rearrangements, output_file)
